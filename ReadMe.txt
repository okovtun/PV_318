...
2)  ??? insert(???);		//вставляет значение в список по заданному индексу
	??? erase(???);			//удаляет значение из списка по заданному индексу

3) Список создает утечку памяти, это нужно исправить;

4) Написать конструктор на заданное количество элементов:
	ForwardList list(5);
